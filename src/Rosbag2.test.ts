...
describe("SqliteNodejs single bag handling", () => {
...
describe("SqliteSqljs single bag handling", () => {
  it("fails when missing metadata.yaml", async () => {
    const bag = new Rosbag2(".", [], (fileEntry) => new SqliteSqljs(fileEntry.file));
    await expect(bag.open()).rejects.toBeDefined();
  });
